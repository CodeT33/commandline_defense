        ...
        ui.horizontal(|ui| {
            ...
            ui.add(egui::Image::new(egui::load::SizedTexture::new(texture_id, [64.0, 64.0])))
                .on_hover_ui(|ui| {
                    ...
                    ui.label(format!("${}", attributes.price));
                    ui.separator();
                    format(ui, attributes.range, "Range");
                    format(ui, attributes.bullet_speed_tps, "Bullet speed");
                    ui.separator();
                    format(ui, attributes.cooldown_ms, "Reload speed");
                    format(ui, attributes.bullet_type.get_attributes().damage, "Bullet damage");
                    format(ui, attributes.bullet_type.get_attributes().pierce, "Bullet pierce");
                    ui.separator();
                    ui.label(
                        (match attributes.targeting_type {
                            TargetingType::Basic => "Basic",
                            TargetingType::Predictive => "Predictive",
                            TargetingType::PredictiveWithLoadBalancing => "Predictive +",
                        })
                        .to_string(),
                    );
                });
